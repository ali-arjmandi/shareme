      ...
      <div className='w-full p-5 flex-1 xl:min-w-620'>
        <div className='flex items-center justify-between'>
          ...
          <a href={pinDetail.destination} target='_blank' rel="noreferrer" >
            {pinDetail.destination?.length > 50 ? `${pinDetail.destination?.slice(8, 50)}...` : pinDetail.destination?.slice(8)}
          </a>
        </div>
        <div>
        ...
        <Link to={`user-profile/${user?._id}`} className='flex gap-2 mt-5 items-center bg-white rounded-lg'>
          ...
          <p className='capitalize'>{pinDetail.postedBy?.userName?.length > 25 ? `${pinDetail.postedBy?.userName.slice(0, 25)}...` : pinDetail.postedBy?.userName}</p>
        </Link>
